...
const {isValid,validEmail} =require("../Regix/Regex")
...
const login=async(req,res)=>{
    try {
        ...
        if (!isValid(email)) { return res.status(401).send({ status: false, message: "author email is not valid string" }) }
        if(!validEmail(email)){return res.status(400).send({status:false,message:"email format is invalid "})}
        if (!isValid(password)) { return res.status(401).send({ status: false, message: "Password is not valid string" }) }
